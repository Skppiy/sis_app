...
// Academic years management with full CRUD operations
...
export default function AcademicYearsSection({ academicYears, onDataChange }) {
  ...
  const handleCreate = async (formData) => {
    try {
      setLoading(true);
      setError("");
      const newYear = await apiPost("/academic-years", formData);
      onDataChange(); // Refresh data in parent
      setShowForm(false);
      console.log("✅ Academic year created:", newYear);
    } catch (err) {
  ...
  const handleEdit = async (formData) => {
    try {
      setLoading(true);
      setError("");
      const updatedYear = await apiPut(`/academic-years/${editingYear.id}`, formData);
      onDataChange(); // Refresh data in parent
      setEditingYear(null);
      console.log("✅ Academic year updated:", updatedYear);
    } catch (err) {
  ...
  const handleDelete = async (year) => {
    ...
    try {
      ...
      onDataChange(); // Refresh data in parent
      console.log("✅ Academic year deleted:", year.name);
    } catch (err) {
  ...
  const handleActivate = async (year) => {
    try {
      ...
      onDataChange(); // Refresh data in parent
      console.log("✅ Academic year activated:", year.name);
    } catch (err) {
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button
                ...
                disabled={loading || year.is_active}
                style={{
                  padding: '4px 8px',
                  background: year.is_active ? '#a0aec0' : '#e53e3e',
                  color: 'white',
                  border: 'none',
                  borderRadius: 4,
                  cursor: (loading || year.is_active) ? 'not-allowed' : 'pointer',
                  fontSize: '0.875rem'
                }}
                title={year.is_active ? "Cannot delete active academic year" : "Delete academic year"}
              >
                Delete
              </button>
            </div>
